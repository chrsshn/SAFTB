...
base.df.john = base.df.john %>%
    mutate(t1 = sapply(strsplit(`1st.Pickup`, "[-]"), "[[", 1),
           midpoint = as.POSIXct(t1, format="%H:%M") + hours(1))

#Replace 1st.pickup, get rid of helper rows and combine both dfs
...
#Impute missing 1st.pickup times - TODO
...
#Add index, score, angular pickup time variables
base.df = base.df %>%
  mutate(index = rep(1:25,4),
         score = 0.4*Tot.Scr.Time + 0.6*Pickups,
         Pickup.1st.angular = (hour(`1st.Pickup`)*60+minute(`1st.Pickup`))/(24*60)*360)
```
...
require(gridExtra)
grid.arrange(box.TST, box.pickups, box.score, ncol = 3)
```
...
require(gridExtra)
grid.arrange(box.TST2, box.pickups2, box.score2, ncol = 3)
